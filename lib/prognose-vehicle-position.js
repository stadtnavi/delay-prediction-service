...
const prognoseAndPublishVehiclePosition = async (db, vehicleId, tVehiclePos) => {
	...
	const vehicleDescriptor = {
		// We don't use the real vehicle ID here in order to have be compatible
		// with the *planned* vehicle positions (which of course don't have a
		// real vehicle ID to use), so that the client can identify a *prognosed*
		// position as the successor of the equivalent *planned* position.
		id: trip_id + '-' + date,
	}
	...
	const vehiclePosition = {
		...
		currentStopSequence: currentOrNextArrDep ? currentOrNextArrDep.stop_sequence : null,
		stopId: currentOrNextArrDep ? currentOrNextArrDep.stop_id : null,
		currentStatus: currentOrNextArrDep ? (
			// todo: INCOMING_AT?
