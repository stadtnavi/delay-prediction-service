...
const publishTripUpdate = async (tU, additionalData = {}) => {
	...
	// gtfs-realtime-bindings (using ProtoBuf.js) expects camel-cased field names,
	// gtfs-rt-differential-to-full-dataset (using gtfs-rt-bindings, using pbf)
	// expects canonical/snake-cased field names.
	// todo: fix this
	// see also https://github.com/derhuerst/gtfs-rt-bindings/issues/6
...
const publishVehiclePosition = async (vP, additionalData = {}) => {
	...
	const agency_name = '1'
	const mode = 'bus' // todo
	const route_id = vP.trip.routeId
	const direction_id = '0' // todo
	const headsign = trip_headsign || 'unknown-headsign'
	const trip_id = vP.trip.tripId
	const next_stop = vP.stopId || 'unknown-next-stop'
	const start_time = vP.trip.startTime || 'unknown-start-time'
