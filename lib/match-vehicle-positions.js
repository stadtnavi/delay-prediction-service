...
const MINIMUM_MATCHING_SCORE = process.env.MINIMUM_MATCHING_SCORE
	? parseInt(process.env.MINIMUM_MATCHING_SCORE)
	: 100
...
const matchRunFromVehiclePositions = async (db, vehicleId, t) => {
	...
	if (bestScore > MINIMUM_MATCHING_SCORE) {
		logger.info({
	...
	const runMatch = {
		vehicleId,
		score: bestScore, tr: bestTr.properties.id,
		secondBestScore, secondBestTr: secondBestTr && secondBestTr.properties.id,
		matchingDuration,
