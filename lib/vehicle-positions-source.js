...
const USE_THINGSBOARD = process.env.READ_VEHICLE_POSITIONS_FROM_STDIN !== 'true'

let subscribeToVehiclePositions = () => {
	return process.stdin.pipe(parse())
}
...
if (USE_THINGSBOARD) {
	...
	subscribeToVehiclePositions = () => {
		...
		const onData = (deviceId, data) => {
			...
			const relevantFields = ['latitude', 'longitude', 'altitude', 'pax']
	...
	logger.info('reading vehicle positions from stdin, not from Thingsboard')
}
